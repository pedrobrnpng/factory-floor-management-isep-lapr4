...
public class ConsumptionMessageTest {
    ...
    public void ensureComsumptionMessageMustHaveProduct() {
        new ConsumptionMessage(MACHINE, TYPE_OF_MESSAGE, DATEHOUR, (Product) null, QUANTITY, DEPOSIT);
    }

    @Test(expected = IllegalArgumentException.class)
    public void ensureComsumptionMessageMustHaveRawMaterial() {
        new ConsumptionMessage(MACHINE, TYPE_OF_MESSAGE, DATEHOUR, (RawMaterial) null, QUANTITY, DEPOSIT);
    }

    @Test(expected = IllegalArgumentException.class)
    public void ensureComsumptionMessageMustHaveQuantityForRawMaterial() {
        new ConsumptionMessage(MACHINE, TYPE_OF_MESSAGE, DATEHOUR, PRODUCT, -1, DEPOSIT);
    }
    
    @Test(expected = IllegalArgumentException.class)
    public void ensureComsumptionMessageMustHaveQuantityForProduct() {
        new ConsumptionMessage(MACHINE, TYPE_OF_MESSAGE, DATEHOUR, PRODUCT, -1, DEPOSIT);
    }

    @Test
    public void ensureComsumptionMessageCanBeCreatedWithoutDepositForRawMaterial() {
        new ConsumptionMessage(MACHINE, TYPE_OF_MESSAGE, DATEHOUR, RAW_MATERIAL, QUANTITY, null);
    }
    
    @Test
    public void ensureComsumptionMessageCanBeCreatedWithoutDepositForProduct() {
        new ConsumptionMessage(MACHINE, TYPE_OF_MESSAGE, DATEHOUR, PRODUCT, QUANTITY, null);
    }
